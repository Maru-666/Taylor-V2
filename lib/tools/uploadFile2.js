...
export default async (conn, buffer, type) => {
    ...
    if (/^tele(graph)?$/i.test(type)) {
        ...
        let data = await req('https://telegra.ph/upload', form, {
        ...
        let data = await req('https://uguu.se/upload.php', form, {
            headers: form.getHeaders()
        })
        return data.files[0].url
    } else if (/^anon(files)?$/i.test(type)) {
        form.append('file', fs.createReadStream(filename))
        let data = await req('https://api.anonfiles.com/upload', form, {
            headers: form.getHeaders()
        })
        ...
        let data = await req(url, form, {
            headers: form.getHeaders()
        })
        return data.replace(/\n/g, '').replace(new RegExp(`.+(https?:\/\/.+/v/[a-zA-Z0-9]+\/file\.html).+`, 'gi'), '$1')
    } else {
        form.append('files[]', fs.createReadStream(filename))
        let data = await req('https://up1.fileditch.com/upload.php', form, {
            headers: form.getHeaders()
        })
        return data.files[0].url
    }
...
async function home() {
    let {
        data
    } = await axios.get('https://www.zippyshare.com')
