...
class BingImageCreator {
	...
	async pollingImgRequest(pollingUrl, onProgress) {
		...
		while (polling) {
			...
			if (body && body.indexOf('errorMessage') === -1) {
				...
				const cancelRequest = onProgress({
					pollingStartTime
				})
				if (cancelRequest) {
	...
	async genImageList(prompt, messageId, removeSizeLimit, onProgress) {
		const {
			pollingUrl
		} = await this.genImagePage(prompt, messageId)
	...
	async genImageIframeSsr(prompt, messageId, onProgress) {
		...
		if (typeof onProgress === 'function') {
			const cancelRequest = onProgress({
				contentIframe: this.createImageIframe(contentUrl)
			})
			if (cancelRequest) {
				throw new Error('Bing Image Creator Error: cancelled')
			}
...
const handler = async (m, {
	...
	try {
		...
		const data = await res.genImageList(text, randomUUID(), true, true);
